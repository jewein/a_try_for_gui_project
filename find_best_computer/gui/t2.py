...
class Window(QMainWindow):
    ...
    def onCpuTextChanged(self):
        if self.ui.lineEdit1.text() == '':
            ...
            data = self.searcher.search(self.ui.lineEdit1.text(), top=20)
            # sys.stdout.flush()
    ...
    def save(self):
        ...
        self.existing_record = self.program_dataframe[
            ...
            (self.program_dataframe['store'] == str(self.store))
            ]
        ...
        shutil.copyfile('program.csv', 'save/program' + datetime.datetime.now().strftime('%Y%m%d_%H%M%S') + '.csv')


    def update(self):
        self.cpu_index = self.ui.comboBox1.currentIndex()  # 获取当前选中的索引
        if self.cpu_index >= 0:  # 如果索引大于等于0，说明有选中的项
            self.ui.lineEdit1.setText(self.cpu_list[self.cpu_index][1])  # 设置文本框的内容
